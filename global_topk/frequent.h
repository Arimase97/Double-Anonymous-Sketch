...
public:
	...
	bool is_topk[2][MAX_bucketNum]; // data structures convenient for query
	...
	void report_topk(unordered_map<int, int>& result, int topk){
		...
		for (int i = 0; i < topk; i++){
			...
			result[item[query_item[i].buc_id][query_item[i].pos].id] = query_topk(item[query_item[i].buc_id][query_item[i].pos].id, tmp, 1, 0);
		}
