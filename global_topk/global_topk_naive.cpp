...
void test_global_topk(int memory, int distrib_cnt, int sketch_id, int topk, int heavy_percent, int blen, char* inp_path, FILE* fout){
    ...
    get_sketch_name(sketch_id, sketch_name);
    fprintf(fout, "%s:\n", sketch_name);
    printf("%s:\n", sketch_name);

    double avg_ARE = 0, avg_F1 = 0, avg_AAE = 0;

    int cdf_distribution[1101];
    ...
    for (int tt = 0; tt < REP_TIME; tt++){
        ...
        //for (i = 0; i < 10; i++){
            ...
            Framework* sketch = new_sketch(sketch_id, memory * heavy_percent / 1600 / blen, blen, memory * (100 - heavy_percent) / 400);
            ...
            sketch->report_topk(*result_topk_map, int(topk / 10 * 1.5), 1);
...
int main(int argc, char* argv[]){
    ...
    for (int t_case = 0; t_case < n_case; t_case++){
        ...
        test_global_topk(memory, distrib_cnt, 4, topk, heavy_percent, blen, inp_path, fout);
        fflush(fout);
    }
    fclose(fin);
    fclose(fout);
