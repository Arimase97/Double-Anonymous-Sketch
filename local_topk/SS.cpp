...
void test_topk(int topk, int rep_time, int tot_flow, int memory){
	...
	double avg_throughput = 0, avg_AAE_unbiased = 0, avg_ARE_under = 0, avg_ARE_unbiased = 0, avg_ARE_over = 0, avg_recall = 0, avg_under = 0, avg_over = 0, avg_equal = 0, avg_deviate = 0, avg_badcase = 0, avg_precision = 0;
	...
	for (int t = 0; t < rep_time; t++){
		...
		avg_precision += s->Check(flow, topk, flow_cnt) / rep_time;
		delete(s);
	}
	fprintf(fout, "%f %f %f %f %f %f %f %f %f %f\n", avg_throughput, avg_AAE_unbiased, avg_ARE_unbiased, avg_under, avg_over, avg_equal, avg_deviate, avg_recall, avg_precision, avg_badcase);
	printf("\n");
}
