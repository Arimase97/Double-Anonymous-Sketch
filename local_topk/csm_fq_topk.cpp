...
int main(int argc, const char* argv[]){
	...
	for (int t = 0; t < n_case; t++){
		...
		int heavy_bucnum = memory * percent * 1024 / 1600 / blen; // 16 bytes per one bucket = 8 items
		int light_bucnum = memory * (100 - percent) * 1024 / 200; // 2 bytes per item
		
		test_topk(blen, topk_val, REP_TIME, min(TOTAL_FLOW, tot_flow_cnt), heavy_bucnum, light_bucnum, 1, fout);
		printf("\n");
	}
	fclose(fin);
    fclose(fout);
